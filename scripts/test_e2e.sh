#!/bin/bash
# NeuroSploit SaaS v2 - End-to-End Test Script
# Tests the complete flow from infrastructure to API

set -e

echo "=============================================="
echo "NeuroSploit SaaS v2 - End-to-End Tests"
echo "=============================================="
...
HEALTH=$(docker exec neurosploit-api curl -s http://localhost:8000/health 2>/dev/null || echo "failed")
...
# Root endpoint
ROOT=$(docker exec neurosploit-api curl -s http://localhost:8000/ 2>/dev/null || echo "failed")
...
if docker images | grep -q "neurosploit-kali"; then
...
# Test nmap in Kali
NMAP_TEST=$(docker run --rm neurosploit-kali:minimal bash -c "nmap --version" 2>&1 | grep -i "nmap" || echo "failed")
...
# Check tables exist
TABLES=$(docker exec neurosploit-postgres psql -U neurosploit -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public'" 2>/dev/null | tr -d ' ')
if [ "$TABLES" -gt 5 ]; then
...
# Check seed data
TENANT=$(docker exec neurosploit-postgres psql -U neurosploit -t -c "SELECT COUNT(*) FROM tenants" 2>/dev/null | tr -d ' ')
if [ "$TENANT" -gt 0 ]; then
...
docker ps --format "  - {{.Names}}: {{.Status}}" | grep neurosploit
echo ""
echo "Next steps:"
echo "  1. Create GitHub repo: https://github.com/mazamizo21/NeuroSploit-SaaS-v2"
